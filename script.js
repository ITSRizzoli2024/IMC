    ...
    try {
        ...
        Papa.parse(text, {
            ...
            complete: (results) => {
                ...
                console.log('Domande processate:', domande[0]); // Verifica l'output
            }
        });
    } catch (error) {
...
function mostraDomanda() {
    ...
    // Debug - vediamo cosa contiene esattamente
    console.log("Opzioni per il tema corrente:", temaCorrente.opzioni);
    
    temaCorrente.opzioni.forEach((opzione, index) => {
        const button = document.createElement('button');
        button.className = 'option-button';
        // Sanitizziamo il testo prima di visualizzarlo
        const testoSanitizzato = opzione.testo
            .replace(/à+/g, 'à')
            .replace(/è+/g, 'è')
            .replace(/é+/g, 'é')
            .replace(/ì+/g, 'ì')
            .replace(/ò+/g, 'ò')
            .replace(/ù+/g, 'ù');
        button.textContent = testoSanitizzato;
